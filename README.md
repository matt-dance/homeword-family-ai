...
| `HOMEWARD_TIMEZONE` | *(unset)* | IANA zone for chat clock hints/cards when the household home location has no timezone. Falls back to `TZ`, then the process local zone (often **UTC in Docker images** unless `TZ` or `/etc/localtime` is set). Set home location in Settings, or export `HOMEWARD_TIMEZONE=America/Denver` (or `TZ`) on the host so Compose can pass it through. |
