...
async def _latest_child_session_id(
    session: AsyncSession, child_id: int, *, quick_chat: bool = False
) -> int | None:
    result = await session.execute(
...
async def _session_is_kid_recoverable(
    ...
    latest_id = await _latest_child_session_id(
...
async def chat_session_messages(
    ...
    if not child:
        raise HTTPException(status_code=404, detail="Chat session not found")
    await _require_chat_access(
        request, child, session, quick_chat=bool(chat_session.quick_chat)
    )
